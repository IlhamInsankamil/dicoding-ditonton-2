...
class _MovieDetailPageState extends State<MovieDetailPage> {
  ...
  void initState() {
    ...
    context.read<MvDetailBloc>().add(GetDetail(widget.id));
    context.read<MvRecommendationBloc>().add(GetRecommendation(widget.id));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocBuilder<MvDetailBloc, MvDetailState>(
        builder: (context, state) {
          if (state is MvDetailLoading) {
            return Center(
              child: CircularProgressIndicator(),
            );
          } else if (state is MvDetailHasData) {
            return SafeArea(
              child: DetailContent(
                state.result,
              ),
            );
          } else if (state is MvDetailError) {
                          ...
                          builder: (context, state) {
                            ...
                            return Column(
                              ...
                              children: [
                                ...
                                ElevatedButton(
                                  onPressed: () async {
                                    ...
                                    context
                                        ...
                                        .add(GetWatchlistStatus(movie.id));
                                    context
                                ...
                                BlocBuilder<MvRecommendationBloc,
                                    MvRecommendationState>(
                                  builder: (context, state) {
                                    if (state is MvRecommendationLoading) {
                                      return Center(
                                        child: CircularProgressIndicator(),
                                      );
                                    } else if (state
                                        is MvRecommendationHasData) {
                                    ...
                                    } else if (state is MvRecommendationError) {
                                      return Expanded(
                                        child: Center(
